...
def header(title):
    ...
    print(color("╔" + "═" * 58 + "╗", "35"))
    print(color(f"║  Budget Tracker v{VERSION}".ljust(59) + "║", "35"))
...
class PasswordManager:
    ...
    def setPassword(self, force=False):
        ...
        hint = input("Enter a password hint (optional, press Enter to skip): ").strip()
        if hint:
        ...
        while attempts > 0:
            ...
            if self.hashPassword(pw) == saved_pw_hash:
                ...
                if attempts > 0:
                    ...
                    if attempts == 1 and os.path.exists(self.hint_file):
                        with open(self.hint_file, "r") as f:
                            hint = f.read().strip()
                        if hint:
                            print(color(f"Hint: {hint}", "33"))
        
        print(color("!!! Too many wrong attempts.", "31"))
        reset = input("Reset password? This will DELETE all saved data. (yes/no): ").strip().lower()
        if reset == "yes":
            self.resetPassword()
            return True
        sys.exit()
    ...
    def changePassword(self):
        ...
        with open(self.password_file, "w") as f:
            f.write(self.hashPassword(pw1))
        hint = input("Enter a new password hint (optional, press Enter to skip): ").strip()
        if hint:
            with open(self.hint_file, "w") as f:
                f.write(hint)
    ...
    def resetPassword(self):
        """Reset password and wipe all data"""
        ...
        for f in os.listdir(data_dir):
            try:
                ...
                pass
        print(color("✔ All data wiped. Please set a new password.", "36"))
        self.session_authenticated = False
        self.setPassword()
...
class BudgetTracker:
    ...
    def backupData(self):
        """Create rotating timestamped backups (keeps last 5)"""
        try:
            if os.path.exists(self.save_file):
                data_dir = os.path.dirname(self.save_file)
                timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
                timestamped = os.path.join(data_dir, f"budget_backup_{timestamp}.json")
                shutil.copy(self.save_file, timestamped)
                # Also keep the legacy single backup for restoreBackup compatibility
                shutil.copy(self.save_file, self.backup_file)
                # Prune old backups, keep latest 5
                backups = sorted([
                    f for f in os.listdir(data_dir)
                    if f.startswith("budget_backup_") and f.endswith(".json") and f != "budget_backup.json"
                ])
    ...
    def viewRecurringExpenses(self):
        """View all recurring expenses"""
        if not self.recurring_expenses:
            print(color("No recurring expenses set.", "33"))
            return
        print(color("\nRECURRING EXPENSES:", "34"))
        for i, r in enumerate(self.recurring_expenses):
            last = r.get("last_added", "never")
            print(f"{i}: {r['category']} | ${r['amount']:.2f} | every {int(r['frequency_days'])} day(s) | last: {last}")
    
    def deleteRecurringExpense(self):
        """Delete a recurring expense"""
        if not self.recurring_expenses:
            print(color("No recurring expenses to delete.", "33"))
            return
        self.viewRecurringExpenses()
        idx = inputInt("\nRecurring expense number to delete: ")
        if idx is None or idx >= len(self.recurring_expenses):
            print(color("!!! Invalid index.", "33"))
            return
        removed = self.recurring_expenses.pop(idx)
        print(color(f"✔ Removed recurring: {removed['category']} - ${removed['amount']:.2f}", "36"))
        self.maybeSave()
    ...
    def editExpense(self):
        ...
        for i, e in enumerate(self.expenses):
            ...
            print(f"{i}: {e['date']} | {e['category']} | ${e['amount']}{note}")
        ...
        print(f"  Current amount: ${self.expenses[idx]['amount']}")
        amt_str = input("  New amount (Enter to keep): ").strip()
        if amt_str:
            try:
                new_amount = float(amt_str)
                if new_amount < 0:
                    print(color("!!! Negative not allowed.", "33"))
                else:
                    self.expenses[idx]["amount"] = new_amount
            except ValueError:
                print(color("!!! Invalid number, keeping original.", "33"))
        
        cur_note = self.expenses[idx].get("note", "")
        print(f"  Current note: {cur_note if cur_note else '(none)'}")
        new_note = input("  New note (Enter to keep): ").strip()
        if new_note:
            self.expenses[idx]["note"] = new_note
    ...
    def viewIncomeHistory(self):
        """View income log"""
        if not self.income_history:
            print(color("No income history recorded.", "33"))
            return
        print(color("\nINCOME HISTORY:", "34"))
        for entry in self.income_history:
            action = "Set to" if entry["type"] == "set" else "Added"
            note = f" | {entry['note']}" if entry.get("note") else ""
            print(f"{entry['date']} | {action} ${entry['amount']:.2f}{note}")
    ...
    def searchExpenses(self):
        """Search expenses by category, date range, or amount range"""
        if not self.expenses:
        ...
        if choice == "1":
            ...
            min_amt = inputFloat("Minimum amount: ")
            if min_amt is None:
                return
            max_str = input("Maximum amount (Enter for no limit): ").strip()
            if max_str:
                try:
                ...
                results = [e for e in self.expenses if e["amount"] >= min_amt]
        else:
        ...
        if results:
            ...
            for e in results:
                note = f" | {e['note']}" if e.get("note") else ""
                print(f"{e['date']} | {e['category']} | ${e['amount']:.2f}{note}")
                total += e["amount"]
            print(color(f"\nTotal: ${total:.2f}", "36"))
        else:
    ...
    def viewMonthly(self):
        ...
        for cat in sorted(by_category.keys()):
            ...
            for e in by_category[cat]:
                note = f" | {e['note']}" if e.get("note") else ""
                print(f"  {e['date']} | ${e['amount']:.2f}{note}")
    ...
    def plotCategoryPie(self):
        ...
        if not category_totals:
            print(color("No expense data available.", "33"))
            return
        
        labels = list(category_totals.keys())
        sizes = [category_totals[c] for c in labels]
    ...
    def exportCSV(self):
        ...
        if not self.expenses:
            print(color("No expenses to export.", "33"))
            return
        
        data_dir = os.path.dirname(self.save_file)
        filename = os.path.join(data_dir, f"expenses_export_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.csv")
        
        try:
            with open(filename, "w", newline="") as f:
                writer = csv.DictWriter(f, fieldnames=["date", "category", "amount", "note", "recurring"])
                writer.writeheader()
                for e in self.expenses:
                    writer.writerow({
                        "date": e.get("date", ""),
            ...
            print(color(f"✔ Exported to: {filename}", "36"))
        except Exception as ex:
            print(color(f"!!! Export failed: {ex}", "31"))
    ...
    def run(self):
        ...
        while True:
            ...
            print(" 8) View Recurring Expenses")
            print(" 9) Delete Recurring Expense")
            
            print(color("\n═══ INCOME & SAVINGS ═══", "32"))
            print("10) Add to Income")
            print("11) Set Total Income")
            print("12) View Income History")
            print("13) Set Savings Goal")
            print("14) View Savings Progress")
              
            print(color("\n═══ REPORTS & VIEWS ═══", "32"))
            print("15) View Categories")
            print("16) View Monthly Expenses")
            print("17) Search Expenses")
            print("18) View Category Chart (Bar)")
            print("19) View Monthly Trend")
            print("20) View Category Chart (Pie)")
            print("21) Export to CSV")
            
            print(color("\n═══ SYSTEM ═══", "36"))
            print("22) Save Data")
            print("23) Load Data")
            print(f"24) Toggle Auto-Save [{'ON' if self.autosave else 'OFF'}]")
            print("25) Restore Backup")
            ...
            if choice == "1": 
                ...
                clear(); header("VIEW RECURRING"); self.viewRecurringExpenses(); pause()
            elif choice == "9": 
                clear(); header("DELETE RECURRING"); self.deleteRecurringExpense(); pause()
            elif choice == "10": 
                clear(); header("ADD INCOME"); self.addIncome(); pause()
            elif choice == "11": 
                clear(); header("SET TOTAL INCOME"); self.setIncome(); pause()
            elif choice == "12": 
                clear(); header("INCOME HISTORY"); self.viewIncomeHistory(); pause()
            elif choice == "13": 
                ...
                self.plotCategoryPie(); pause()
            elif choice == "21": 
                clear(); header("EXPORT CSV"); self.exportCSV(); pause()
            elif choice == "22": 
                self.saveData(); pause()
            elif choice == "23": 
                self.loadData(); pause()
        ...
        if self.autosave: 
            self.saveData()
        print(color(f"✔ Exiting Budget Tracker v{VERSION}", "36"))
